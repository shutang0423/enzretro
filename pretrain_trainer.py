...
def _topk_hit(logits: torch.Tensor,
              targets: torch.Tensor,
              valid_mask: torch.Tensor,
              k: int):
    """返回 (命中数, 有效样本数)，用于跨 batch 累加后再除
    
    注意：k 自动 clamp 到 logits.size(-1)，防止节点数 < k 时报错
    """
    if not valid_mask.any():
        return 0, 0
    logits_v  = logits[valid_mask]                                    # [M, nodes]
    targets_v = targets[valid_mask]                                   # [M]
    k_safe    = min(k, logits_v.size(-1))                             # ← 防止 k > nodes
    topk_idx  = logits_v.topk(k_safe, dim=-1).indices                # [M, k]
    hit       = (topk_idx == targets_v.unsqueeze(1)).any(dim=1)      # [M]
    return hit.sum().item(), valid_mask.sum().item()


def _seq_acc_counts(logits: torch.Tensor,
                    targets: torch.Tensor,
                    pad_id: int):
    """返回 (正确token数, 总token数, 完整序列正确数, 总序列数)"""
    pred    = logits.argmax(dim=-1)           # [N, L]
    non_pad = targets != pad_id               # [N, L]

    correct_tokens = ((pred == targets) & non_pad).sum().item()
    total_tokens   = non_pad.sum().item()

    seq_correct    = ((pred == targets) | ~non_pad).all(dim=1).sum().item()
    total_seqs     = targets.size(0)

    return correct_tokens, total_tokens, seq_correct, total_seqs
...
class PretrainTrainer:
    ...
    def __init__(self, actor: ActorNetwork, cfg, device: torch.device):
        ...
        # TensorBoard
        self.writer = SummaryWriter(log_dir=str(cfg.path.TB_DIR))
    ...
    def _build_action_weight(self, data_loader) -> torch.Tensor:
        """统计训练集 action 分布，计算 inverse-frequency 权重"""
        counts = torch.zeros(self.cfg.model.NUM_ACTIONS)
        for batch in data_loader:
            for aid in batch["target_actions"].view(-1):
                counts[aid.item()] += 1
        # 出现次数为 0 的类给权重 0（避免 inf）
    ...
    def compute_loss(self, batch: Dict) -> Dict[str, torch.Tensor]:
        ...
        action_loss = safe_loss(
            F.cross_entropy(action_logits, 
                            target_actions,
                            weight=self.action_weight,
                            ignore_index=self.cfg.model.PAD_ACTION_ID), 
            "action"
        )
    ...
    def train_epoch(self, dataloader) -> Dict[str, float]:
        self.actor.train()
        self.uw.train()
        self.action_weight = self._build_action_weight(dataloader)
    ...
    # ══════════════════════════════════════════════════════════════════
    #  PretrainTrainer.test()  — 放在类内部 eval_epoch() 之后
    # ══════════════════════════════════════════════════════════════════
    ...
    def test(self,
            ...
            topk_list: list = [1, 3, 5]) -> Dict[str, float]:
        """加载 best model，在测试集上评估多维度指标

        修复：src/tgt logits 的 max_nodes 维度在不同 batch 间不同，
            不能跨 batch 拼接，改为 batch 内计算命中数后累加。

        指标:
        loss/{loss,action_loss,src_loss,tgt_loss,label_loss}
        action_acc
        src_top{k}_acc  (k ∈ topk_list，仅 valid 步骤)
        tgt_top{k}_acc
        label_token_acc
        label_seq_acc
        joint_step_acc  (action + src_top1 + tgt_top1 + label_seq 全对)
        """
        # ── 加载 checkpoint ────────────────────────────────────────
        load_path = Path(ckpt_path or self.cfg.path.CKPT_BEST_MODEL_FILE)
        if not load_path.exists():
            raise FileNotFoundError(f"Checkpoint not found: {load_path}")

        ckpt = torch.load(load_path, map_location=self.device, weights_only=False)
        self.actor.load_state_dict(ckpt["model"])
        if "uw" in ckpt and hasattr(self, "uw"):
            self.uw.load_state_dict(ckpt["uw"])
        logger.info(
            f"Loaded best model from {load_path}  "
        ...
        # ── 累加器 ────────────────────────────────────────────────
        sum_loss   = {k: 0.0 for k in
                    ["loss","action_loss","src_loss","tgt_loss","label_loss"]}
        n_batches  = 0
        ...
        # pointer：每个 k 分别累加 (hits, total)
        src_hits  = {k: 0 for k in topk_list}
        src_total = {k: 0 for k in topk_list}
        tgt_hits  = {k: 0 for k in topk_list}
        tgt_total = {k: 0 for k in topk_list}
        ...
        for batch in test_loader:
            # ── 数据搬运 ──────────────────────────────────────────
            pyg            = batch["pyg_batch"].to(self.device)
            target_actions = batch["target_actions"].to(self.device)   # [N]
            target_srcs    = batch["target_srcs"].to(self.device)      # [N]
            target_tgts    = batch["target_tgts"].to(self.device)      # [N]
            dec_inputs     = batch["decoder_inputs"].to(self.device)   # [N, L]
            dec_targets    = batch["decoder_targets"].to(self.device)  # [N, L]
            src_valid      = batch["src_valid_mask"].to(self.device)   # [N]
            tgt_valid      = batch["tgt_valid_mask"].to(self.device)   # [N]
            step_to_sample = batch["step_to_sample"].to(self.device)   # [N]
            ...
            # src_logits: [N, max_nodes_this_batch]  ← 每 batch 不同！

            # ── loss ──────────────────────────────────────────────
            losses = self.compute_loss(batch)
            for k in sum_loss:
                sum_loss[k] += losses[k].item()
            n_batches += 1

            # ── 1. Action Accuracy ────────────────────────────────
            action_pred       = action_logits.argmax(dim=-1)           # [N]
            action_correct_n += (action_pred == target_actions).sum().item()
            action_total_n   += target_actions.size(0)

            # ── 2. Pointer Top-k（batch 内计算，避免 cat 维度冲突）──
            N_nodes = src_logits.size(-1)   # 本 batch 的 max_nodes

            # 有效 mask：src_valid 且索引在范围内
            src_v = src_valid & (target_srcs >= 0) & (target_srcs < N_nodes)
            tgt_v = tgt_valid & (target_tgts >= 0) & (target_tgts < N_nodes)

            for k in topk_list:
                h, t = _topk_hit(src_logits, target_srcs, src_v, k)
                src_hits[k]  += h
                src_total[k] += t
                h, t = _topk_hit(tgt_logits, target_tgts, tgt_v, k)
                tgt_hits[k]  += h
                tgt_total[k] += t

            # ── 3. Label Accuracy ─────────────────────────────────
            tc, tt, sc, st = _seq_acc_counts(label_logits, dec_targets, pad_id)
            lbl_token_correct += tc
            lbl_token_total   += tt
            lbl_seq_correct   += sc
            lbl_seq_total     += st

            # ── 4. Joint Step Accuracy ────────────────────────────
            act_ok   = (action_pred == target_actions)                  # [N]

            # src/tgt：valid 位置必须 top-1 命中；invalid 位置直接算对
            src_pred = src_logits.argmax(dim=-1)
            tgt_pred = tgt_logits.argmax(dim=-1)
            src_ok   = (src_pred == target_srcs) | ~src_v              # [N]
            tgt_ok   = (tgt_pred == target_tgts) | ~tgt_v              # [N]

            # label：整条序列完全匹配
            lbl_pred = label_logits.argmax(dim=-1)                     # [N, L]
            non_pad  = dec_targets != pad_id
            lbl_ok   = ((lbl_pred == dec_targets) | ~non_pad).all(dim=1)  # [N]

            joint_correct += (act_ok & src_ok & tgt_ok & lbl_ok).sum().item()
            joint_total   += target_actions.size(0)
        ...
        for k, v in sum_loss.items():
            metrics[f"loss/{k}"] = v / max(n_batches, 1)

        metrics["action_acc"] = action_correct_n / max(action_total_n, 1)

        for k in topk_list:
            metrics[f"src_top{k}_acc"] = src_hits[k] / max(src_total[k], 1)
            metrics[f"tgt_top{k}_acc"] = tgt_hits[k] / max(tgt_total[k], 1)

        metrics["label_token_acc"] = lbl_token_correct / max(lbl_token_total, 1)
        metrics["label_seq_acc"]   = lbl_seq_correct   / max(lbl_seq_total,   1)
        metrics["joint_step_acc"]  = joint_correct      / max(joint_total,     1)
        ...
        logger.info(sep)
        logger.info(
            f"  Loss      │ total={metrics['loss/loss']:.4f}  "
            f"action={metrics['loss/action_loss']:.4f}  "
            f"src={metrics['loss/src_loss']:.4f}  "
            f"tgt={metrics['loss/tgt_loss']:.4f}  "
            f"label={metrics['loss/label_loss']:.4f}"
        )
        logger.info(f"  Action    │ acc={metrics['action_acc']:.4f}  "
                    f"({metrics['action_acc']*100:.2f}%)")
        for k in topk_list:
            logger.info(
                f"  Src Top-{k} │ {metrics[f'src_top{k}_acc']:.4f}"
                f"  ({metrics[f'src_top{k}_acc']*100:.2f}%)"
                f"  [{src_hits[k]}/{src_total[k]}]"
            )
        for k in topk_list:
            logger.info(
                f"  Tgt Top-{k} │ {metrics[f'tgt_top{k}_acc']:.4f}"
                f"  ({metrics[f'tgt_top{k}_acc']*100:.2f}%)"
                f"  [{tgt_hits[k]}/{tgt_total[k]}]"
            )
        logger.info(f"  Label Tok │ {metrics['label_token_acc']:.4f}"
                    f"  ({metrics['label_token_acc']*100:.2f}%)")
        logger.info(f"  Label Seq │ {metrics['label_seq_acc']:.4f}"
                    f"  ({metrics['label_seq_acc']*100:.2f}%)")
        logger.info(f"  Joint     │ {metrics['joint_step_acc']:.4f}"
                    f"  ({metrics['joint_step_acc']*100:.2f}%)"
                    f"  [{joint_correct}/{joint_total}]")
        logger.info(sep)

        # ── TensorBoard ───────────────────────────────────────────
        for name, val in metrics.items():
            if val == val:   # 跳过 NaN
                self.writer.add_scalar(f"test/{name}", val, 0)
...
if __name__ == "__main__":
    ...
    logger.info("Test pretrain model")
    test_loader = build_dataloader(json_path=str(cfg.path.RL_TEST_DATA_FILE), tokenizer=tokenizer,
                                        batch_size=cfg.model.BATCH_SIZE, shuffle=False, mode="pretrain")
    # ── 训练结束后自动测试 ──────────────────────────────────────
    test_metrics = pretrain_trainer.test(
        test_loader=test_loader,
        topk_list=[1, 3, 5, 10],       # 可自定义 k 值
        ckpt_path=cfg.path.CKPT_LAST_MODEL_FILE  # 不传则自动加载 best
    )
