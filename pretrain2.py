import torch
import torch.nn as nn
import torch.optim as optim
from torch.optim.lr_scheduler import LambdaLR
from torch_geometric.loader import DataLoader
from torch_geometric.utils import to_dense_batch
from tqdm import tqdm

from config.config import MODEL_CONFIG, TRAIN_CONFIG, PATH_CONFIG
from tokenizer.tokenizer import LabelTokenizer
from data.ssr_graph_pretrain_dataset import SSRGraphDataset
from model.actor_pretrainer import ActorPretrainer

C  = MODEL_CONFIG
TC = TRAIN_CONFIG
PC = PATH_CONFIG

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# ══════════════════════════════════════════════════════════════════════════
#  Uncertainty Weighting（自动 loss 权重，可学习参数）
#  论文: "Multi-Task Learning Using Uncertainty to Weigh Losses" Kendall 2018
# ══════════════════════════════════════════════════════════════════════════
class UncertaintyWeighting(nn.Module):
    """
    每个任务维护一个可学习的 log_sigma，
    loss_i_weighted = exp(-log_sigma_i) * loss_i + log_sigma_i
    log_sigma 越大 → 该任务权重越小（自动降权量级大的任务）
    """
    def __init__(self, num_tasks: int):
        super().__init__()
        # 初始化为 0，即初始权重均为 1
        self.log_sigma = nn.Parameter(torch.zeros(num_tasks))

    def forward(self, *losses):
        assert len(losses) == self.log_sigma.shape[0]
        weighted = []
        for i, loss in enumerate(losses):
            # exp(-s)*loss + s，s = log_sigma
            w = torch.exp(-self.log_sigma[i])
            weighted.append(w * loss + self.log_sigma[i])
        return sum(weighted), [l.item() for l in weighted]

    def weights(self):
        """返回当前各任务的实际权重（用于监控）"""
        return torch.exp(-self.log_sigma).detach().cpu().tolist()


# ══════════════════════════════════════════════════════════════════════════
#  分阶段课程训练配置
#  stage: (name, 训练哪些任务, 冻结哪些模块, 持续 epoch 数)
# ══════════════════════════════════════════════════════════════════════════
STAGES = [
    {
        "name"    : "Stage1-Action",
        "epochs"  : TC.get("stage1_epochs", 10),
        "tasks"   : ["action"],                          # 只训动作预测
        "freeze"  : ["pointer_network", "label_decoder"],# 冻结其余
    },
    {
        "name"    : "Stage2-Pointer",
        "epochs"  : TC.get("stage2_epochs", 15),
        "tasks"   : ["src", "tgt"],                      # 训指针网络
        "freeze"  : ["action_predictor", "label_decoder"],
    },
    {
        "name"    : "Stage3-Label",
        "epochs"  : TC.get("stage3_epochs", 15),
        "tasks"   : ["label"],                           # 训 label 解码
        "freeze"  : ["action_predictor", "pointer_network"],
    },
    {
        "name"    : "Stage4-Joint",
        "epochs"  : TC.get("stage4_epochs", 60),
        "tasks"   : ["action", "src", "tgt", "label"],   # 全部解冻，联合微调
        "freeze"  : [],
    },
]


def set_freeze(model, freeze_modules: list):
    """冻结/解冻指定子模块"""
    # 先全部解冻
    for p in model.parameters():
        p.requires_grad = True
    # 再冻结指定模块
    for name in freeze_modules:
        module = getattr(model, name, None)
        if module is not None:
            for p in module.parameters():
                p.requires_grad = False


def make_optimizer_scheduler(model, uw, total_steps, lr, warmup_ratio=0.1):
    """为当前阶段创建新的 optimizer + scheduler"""
    params = (
        [p for p in model.parameters() if p.requires_grad]
        + list(uw.parameters())
    )
    optimizer    = optim.AdamW(params, lr=lr, weight_decay=1e-2)
    warmup_steps = int(total_steps * warmup_ratio)

    def lr_lambda(step):
        if step < warmup_steps:
            return float(step) / max(1, warmup_steps)
        progress = float(step - warmup_steps) / max(1, total_steps - warmup_steps)
        return max(0.0, 0.5 * (1.0 + torch.cos(
            torch.tensor(3.14159 * progress)).item()))

    scheduler = LambdaLR(optimizer, lr_lambda)
    return optimizer, scheduler


def safe_loss(logits, target, criterion, name):
    loss = criterion(logits, target)
    if torch.isnan(loss) or torch.isinf(loss):
        print(f"\n  [NaN] {name} logits:[{logits.min():.2f},{logits.max():.2f}]"
              f" target:[{target.min()},{target.max()}]")
        return torch.tensor(0.0, device=logits.device, requires_grad=True)
    return loss


# ══════════════════════════════════════════════════════════════════════════
#  数据 & 模型初始化
# ══════════════════════════════════════════════════════════════════════════
tokenizer  = LabelTokenizer(vocab_file=PC["vocab_file"])
dataset    = SSRGraphDataset(
    json_path    = PC["train_data"],
    tokenizer    = tokenizer,
    max_seq_len  = C["max_seq_len"],
    max_hist_len = C["max_hist_len"],
)
dataloader = DataLoader(dataset, batch_size=TC["batch_size"], shuffle=True)

vocab_size = tokenizer.get_vocab_size()
model      = ActorPretrainer(vocab_size=vocab_size).to(device)

# Embedding 初始化
def init_weights(m):
    if isinstance(m, nn.Embedding):
        nn.init.normal_(m.weight, mean=0.0, std=0.02)
    elif isinstance(m, nn.Linear):
        nn.init.xavier_uniform_(m.weight)
        if m.bias is not None:
            nn.init.zeros_(m.bias)
model.apply(init_weights)

# 4 个任务的自动权重模块
uw = UncertaintyWeighting(num_tasks=4).to(device)

criterion_cls = nn.CrossEntropyLoss(ignore_index=-1)
criterion_seq = nn.CrossEntropyLoss(ignore_index=tokenizer.pad_token_id)

# ══════════════════════════════════════════════════════════════════════════
#  保存最优模型（仅 Stage4 联合阶段追踪）
# ══════════════════════════════════════════════════════════════════════════
best_loss      = float("inf")
CKPT_PATH      = PC.get("ckpt_path", "best_model.pt")

def save_best(avg_loss, epoch_tag):
    global best_loss
    if avg_loss < best_loss:
        best_loss = avg_loss
        torch.save({
            "model":     model.state_dict(),
            "uw":        uw.state_dict(),
            "loss":      best_loss,
            "epoch_tag": epoch_tag,
        }, CKPT_PATH)
        print(f"  ✅ Best model saved  loss={best_loss:.4f}  → {CKPT_PATH}")

# ══════════════════════════════════════════════════════════════════════════
#  分阶段训练主循环
# ══════════════════════════════════════════════════════════════════════════
global_epoch = 0

for stage in STAGES:
    stage_name    = stage["name"]
    stage_epochs  = stage["epochs"]
    active_tasks  = set(stage["tasks"])
    is_joint     = (stage_name == "Stage4-Joint")   # ← 只在 Stage4 保存

    print(f"\n{'='*60}")
    print(f"  {stage_name}  |  tasks: {active_tasks}  |  epochs: {stage_epochs}")
    print(f"{'='*60}")

    # 设置冻结
    set_freeze(model, stage["freeze"])

    # 为本阶段创建 optimizer & scheduler
    total_steps = stage_epochs * len(dataloader)
    # Stage4 联合微调用更小的 lr
    stage_lr = TC["lr"] / 5 if stage["name"] == "Stage4-Joint" else TC["lr"]
    optimizer, scheduler = make_optimizer_scheduler(
        model, uw, total_steps, lr=stage_lr
    )

    for epoch in range(stage_epochs):
        global_epoch += 1
        sum_loss = sum_act = sum_src = sum_tgt = sum_lbl = 0.0
        num_batches = 0

        pbar = tqdm(
            dataloader,
            desc  = f"{stage_name} Ep[{epoch+1}/{stage_epochs}]",
            ncols = 150,
            leave = True,
        )

        model.train()
        for batch in pbar:
            batch = batch.to(device)
            optimizer.zero_grad()

            # ── 图编码（每阶段都需要）─────────────────────────────────
            node_embeddings, graph_embedding = model.graph_encoder(
                batch.x, batch.edge_index, batch.batch
            )
            graph_state   = model.state_proj(graph_embedding)
            decoder_state = model.state_tracker(batch.history_actions, graph_state)

            target_action = batch.target_action.squeeze(-1)
            target_src    = batch.target_src.squeeze(-1)
            target_tgt    = batch.target_tgt.squeeze(-1)

            # ── 各任务 loss（不在 active_tasks 中则 detach 不参与梯度）──
            # Action
            action_logits = model.action_predictor(decoder_state)
            loss_action   = safe_loss(action_logits, target_action,
                                      criterion_cls, "action")
            if "action" not in active_tasks:
                loss_action = loss_action.detach()

            # Src / Tgt
            dense_nodes, node_mask_bool = to_dense_batch(node_embeddings, batch.batch)
            node_mask = ~node_mask_bool
            src_logits, tgt_logits = model.pointer_network(
                decoder_state, dense_nodes, target_action,
                target_src_idx = target_src,
                node_mask      = node_mask,
            )
            N         = src_logits.size(-1)
            valid_src = (target_src >= 0) & (target_src < N)
            valid_tgt = (target_tgt >= 0) & (target_tgt < N)

            loss_src = (safe_loss(src_logits[valid_src], target_src[valid_src],
                                  criterion_cls, "src")
                        if valid_src.any() else
                        torch.tensor(0.0, device=device, requires_grad=True))
            loss_tgt = (safe_loss(tgt_logits[valid_tgt], target_tgt[valid_tgt],
                                  criterion_cls, "tgt")
                        if valid_tgt.any() else
                        torch.tensor(0.0, device=device, requires_grad=True))

            if "src" not in active_tasks:
                loss_src = loss_src.detach()
            if "tgt" not in active_tasks:
                loss_tgt = loss_tgt.detach()

            # Label
            decoder_input_seq = batch.target_label[:, :-1]
            label_target_seq  = batch.target_label[:, 1:]
            label_logits      = model.label_decoder(
                decoder_state, target_action, decoder_input_seq
            )
            loss_label = safe_loss(
                label_logits.reshape(-1, vocab_size),
                label_target_seq.reshape(-1),
                criterion_seq, "label"
            )
            if "label" not in active_tasks:
                loss_label = loss_label.detach()

            # ── Uncertainty Weighting 自动平衡权重 ────────────────────
            loss, _ = uw(loss_action, loss_src, loss_tgt, loss_label)

            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
            optimizer.step()
            scheduler.step()

            # ── 统计 ──────────────────────────────────────────────────
            la, ls, lt, ll = (loss_action.item(), loss_src.item(),
                              loss_tgt.item(),    loss_label.item())
            tot = loss.item()
            sum_loss += tot; sum_act += la; sum_src += ls
            sum_tgt  += lt;  sum_lbl += ll; num_batches += 1

            w = uw.weights()   # 当前自动权重
            pbar.set_postfix({
                "tot"  : f"{tot:.3f}",
                "act"  : f"{la:.3f}({w[0]:.2f})",
                "src"  : f"{ls:.3f}({w[1]:.2f})",
                "tgt"  : f"{lt:.3f}({w[2]:.2f})",
                "label": f"{ll:.3f}({w[3]:.2f})",
                "lr"   : f"{scheduler.get_last_lr()[0]:.1e}",
            })

        n = max(num_batches, 1)
        w = uw.weights()
        avg     = sum_loss / n
        print(f"  ▶ [Ep {global_epoch}] {stage_name} avg | "
              f"total:{avg:.4f} act:{sum_act/n:.4f}(w={w[0]:.2f}) "
              f"src:{sum_src/n:.4f}(w={w[1]:.2f}) "
              f"tgt:{sum_tgt/n:.4f}(w={w[2]:.2f}) "
              f"label:{sum_lbl/n:.4f}(w={w[3]:.2f})")

        # ── 仅 Stage4 保存最优 ckpt ──────────────────────────────────
        if is_joint:
            save_best(avg, f"ep{global_epoch}")


